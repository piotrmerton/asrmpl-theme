...
{
    public static function getSerieATable(
        $full = true,
        $season = CURRENT_SEASON
    ) {
        $data = self::getData(
            "https://v3.football.api-sports.io/standings?league=135&season=" .
                $season
        );

        if (!$data or $data->results == 0) {
            return false;
        }
        ...
        foreach ($standings as $row) {
            $row->crest = self::getImage(
                $row->team->logo,
                sanitize_title($row->team->name)
            );
        }
    ...
    public static function getSerieATopScorers(
        $limit = 6,
        $season = CURRENT_SEASON
    ) {
        $data = self::getData(
            "https://v3.football.api-sports.io/players/topscorers?league=135&season=" .
                $season
        );
        ...
        foreach ($topscorers as $row) {
            $row->crest = self::getImage(
                $row->statistics[0]->team->logo,
                sanitize_title($row->statistics[0]->team->name)
            );
        }
    ...
    {
        $data = self::getData(
            "https://v3.football.api-sports.io/players/squads?team=" . $id_team,
            604800
        );

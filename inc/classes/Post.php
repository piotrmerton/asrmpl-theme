...
class Post {
    ...
    public static function getMetaForRestApi($id_post) {
        ...
        $post_age = human_time_diff( get_the_date( 'U', $id_post ), current_time( 'timestamp' ) ) . ' ' . __('temu', 'theme');
        $comments_number = get_comments_number($id_post);
    
        if( $comments_number  == 1) {
            $comment_label = __('komentarz');
        } elseif (  in_array( substr($comments_number, -1), array('2', '3', '4') ) ) {
        ...
        $meta_fields['comments_display'] = $comments_number . ' ' . $comment_label;
        $meta_fields['date_display'] = get_post_time('U', false, $id_post) > date('U') - 60*60*24 ? $post_age : get_the_date('F j Y, H:i', $id_post);

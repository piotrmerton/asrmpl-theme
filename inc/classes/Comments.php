...
class Comments {
    ...
    public static function getMetaForRestApi($id_comment) {
        ...
        $comment_age = human_time_diff( get_comment_date( 'U', $id_comment), current_time( 'timestamp' ) ) . ' ' . __('temu', 'theme');
        ...
        $meta_fields['date_display'] = get_comment_date('U', $id_comment) > date('U') - 60*60*24 ? $comment_age : get_comment_date('F j Y, H:i', $id_comment);
    ...
    public static function renderCommentCustomMarkup($comment, $args, $depth) {
        ...
        $context = array(
            ...
            'comment' => $comment,
            'comment_text' => apply_filters( 'comment_text', strip_tags(get_comment_text( $comment )) ),
            ...
            'ago' => human_time_diff( get_comment_time( 'U' ), current_time( 'timestamp' ) ),
        );

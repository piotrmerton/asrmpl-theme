...
function theme_enqueue_frontend_scripts() {
    ...
    if( is_home() ) wp_enqueue_script( 'index' );
...
function theme_remove_admin_menus() {
    ...
    if( defined('WP_LOCALHOST') && WP_LOCALHOST != true) remove_menu_page( 'theme-my-login' );
...
function theme_add_custom_taxonomy_filter_to_edit_screen ($taxonomy) {
    ...
    echo '<label class="screen-reader-text" for="'.$taxonomy.'">Filter by ' .
...
function theme_update_cpt_game_title( $post_id ) {
    ...
    if( get_post_type() != 'game' OR !empty($_POST['post_title']) ) return;
...
function theme_delete_transients($post_id, $post) {
    if($post->post_type == 'game') {
        ...
        global $wpdb;
        $wpdb->query( "DELETE FROM `$wpdb->options` WHERE `option_name` LIKE ('_transient_schedule_%')" );
        $wpdb->query( "DELETE FROM `$wpdb->options` WHERE `option_name` LIKE ('_transient_timeout_schedule_%')" );
    } 
...
function theme_reorder_comments($query) {
    ...
    if( !comments_open() or is_admin() ) return;

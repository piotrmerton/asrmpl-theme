...
export default function posts(selector, api_url) {
  ...
  nodes.forEach((node) => {
    const grid =
      node.dataset.grid != undefined ? node.dataset.grid : "horizontal"; //either horizontal or vertical, will be applied to css classes
    const show_meta =
      node.dataset.showMeta != undefined ? node.dataset.showMeta : true;

    const id_current_post =
      node.dataset.currentPostId != undefined
        ? node.dataset.currentPostId
        : false;
    const id_tag = node.dataset.tagId != undefined ? node.dataset.tagId : false;
    const id_category =
      node.dataset.categoryId != undefined ? node.dataset.categoryId : false;
    const id_game =
      node.dataset.gameId != undefined ? node.dataset.gameId : false;
    let limit = node.dataset.limit != undefined ? node.dataset.limit : false;

    let url = new URL(api_url);

    if (id_current_post)
      url.searchParams.append("exclude", parseInt(id_current_post));

    if (id_tag) url.searchParams.append("tags", parseInt(id_tag));

    if (id_category)
      url.searchParams.append("categories", parseInt(id_category));

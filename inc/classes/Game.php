...
class Game {
    ...
    public static function generateSchedule(array $games) {
        ...
        foreach($games as $game) {
            ...
            $game->next = isset($has_finished) && $has_finished == true && !$is_final_time ? true : false;
            if($game->next) $schedule['has_next_game'] = true;
    ...
    public static function getData(int $id_game, $broadcaster = false) {
         ...
         */        
        $meta['teams'] = array(
            'home' => Team::getTeamData( get_post_meta($id_game, 'teams_home', true) ),
            'away' => Team::getTeamData( get_post_meta($id_game, 'teams_away', true) ),
        );
    ...
    public static function hasRelatedPosts(int $id_game) : bool {
        ...
        if($id_match_report and is_numeric($id_match_report)) {
            $args['post__not_in'] = array($id_match_report);
        }
    ...
    public static function getCompetition(int $id_game, $logo = false) {
        ...
        $competition->link = get_term_link((int)$id_competition, 'competition');

        if($logo) $competition->logo = get_field('logo', 'competition_' . $id_competition);
    ...
    public static function getBroadcaster(int $id_game) {
        ...
        if(!$id_broadcaster) return false;
